...
void Store::cleanup_expired() {
    std::cout << "Cleaning started" << std::endl;
    std::lock_guard<std::mutex> lock(store_mutex);

    if (auto it = expiry.begin(); it != expiry.end()) {
        REDIS_LOG(INFO, "Expiry Key %s value_time %s", it->first.c_str(), time_to_string(it->second).c_str());
 ...
 * ============================================================
 */
RedisStatus Store::append_to_aof(const std::string& command) {
    if (!aof_file.is_open())
 ...
 * ============================================================
 */
RedisStatus Store::replay_aof(const std::string& filename) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        REDIS_LOG(INFO, "FAIL AOF_REPLAY_OPEN file=%s", filename.c_str());
        return REDIS_STATUS_FAILURE;
    }
...
std::string Store::set_with_ttl(const std::string &key, const std::string &value, const std::string& ttl_seconds) {
    REDIS_LOG(INFO, "key %s value %s ttl_sec %s", key.c_str(), value.c_str(), ttl_seconds.c_str());
    std::lock_guard<std::mutex> lock(store_mutex);
    if (is_expired(key)) {
        db.erase(key);
        expiry.erase(key);
